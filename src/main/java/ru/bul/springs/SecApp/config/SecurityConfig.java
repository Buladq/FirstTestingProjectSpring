...
//        http.csrf().disable()//откл защиту от межсайтовый подделки запросов
            http.authorizeRequests()
                    .antMatchers("/admin/showADmin").hasRole("ADMIN")//доступ только админу
                .antMatchers("/auth/login","/auth/registration" ,"/error","/testing/here").permitAll() //пишу что к этой странице могут попасть все
//                .anyRequest().authenticated() //на все страницы не впускаю не авторизованных пользователей
                ...
                .defaultSuccessUrl("/testing/here",true) //куда направлять после успешного входа
